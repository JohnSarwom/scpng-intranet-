---
source_file: "src\integrations\microsoft\msalService.ts"
type: "code"
community: "Module 32"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Module_32
---

# msalService.ts

## Connections
- [[MicrosoftLoginButton.tsx]] - `imports_from` [INFERRED]
- [[callMsGraphApi()]] - `contains` [EXTRACTED]
- [[diagnoseMsalIssues()]] - `contains` [EXTRACTED]
- [[getAccessToken()]] - `contains` [EXTRACTED]
- [[getAccount()]] - `contains` [EXTRACTED]
- [[getMsalInstance()]] - `contains` [EXTRACTED]
- [[getUserProfile()]] - `contains` [EXTRACTED]
- [[handleRedirectResponse()]] - `contains` [EXTRACTED]
- [[loginRedirect()]] - `contains` [EXTRACTED]
- [[loginWithMicrosoft()]] - `contains` [EXTRACTED]
- [[logout()]] - `contains` [EXTRACTED]
- [[setMsalInstance()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Module_32
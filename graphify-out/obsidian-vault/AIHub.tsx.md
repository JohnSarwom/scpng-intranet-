---
source_file: "src\pages\AIHub.tsx"
type: "code"
community: "AI Hub Chat"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/AI_Hub_Chat
---

# AIHub.tsx

## Connections
- [[fetchAiSettings()_4]] - `contains` [EXTRACTED]
- [[fetchUploadedFiles()]] - `contains` [EXTRACTED]
- [[getAiModes()]] - `contains` [EXTRACTED]
- [[handleClearChat()_4]] - `contains` [EXTRACTED]
- [[handleCopyMessage()]] - `contains` [EXTRACTED]
- [[handleFollowUpClick()_4]] - `contains` [EXTRACTED]
- [[handleKnowledgeUpload()]] - `contains` [EXTRACTED]
- [[handleLibraryQuestionSelect()]] - `contains` [EXTRACTED]
- [[handleSaveAiSettings()]] - `contains` [EXTRACTED]
- [[handleScroll()_6]] - `contains` [EXTRACTED]
- [[handleScrollToBottomClick()]] - `contains` [EXTRACTED]
- [[handleSendChatMessage()]] - `contains` [EXTRACTED]
- [[handleStopGeneration()_4]] - `contains` [EXTRACTED]
- [[handleTestAiConnection()]] - `contains` [EXTRACTED]
- [[if()_5]] - `contains` [EXTRACTED]
- [[openUploadModalForArea()]] - `contains` [EXTRACTED]
- [[scrollToBottom()_5]] - `contains` [EXTRACTED]
- [[typeNextChar()_4]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/AI_Hub_Chat
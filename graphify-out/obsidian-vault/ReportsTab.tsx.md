---
source_file: "src\components\unit-tabs\ReportsTab.tsx"
type: "code"
community: "Reports Tab"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Reports_Tab
---

# ReportsTab.tsx

## Connections
- [[ReportPreview()]] - `contains` [EXTRACTED]
- [[buildTitle()]] - `contains` [EXTRACTED]
- [[computeMetrics()]] - `contains` [EXTRACTED]
- [[filterKPIs()]] - `contains` [EXTRACTED]
- [[filterKRAs()]] - `contains` [EXTRACTED]
- [[filterObjectives()]] - `contains` [EXTRACTED]
- [[filterTasks()]] - `contains` [EXTRACTED]
- [[formatDate()_5]] - `contains` [EXTRACTED]
- [[getDateRange()]] - `contains` [EXTRACTED]
- [[handleCopyMetadata()]] - `contains` [EXTRACTED]
- [[handleDeleteSchedule()]] - `contains` [EXTRACTED]
- [[handleEditSchedule()]] - `contains` [EXTRACTED]
- [[handleExportCSV()]] - `contains` [EXTRACTED]
- [[handleGenerate()]] - `contains` [EXTRACTED]
- [[handlePrint()_1]] - `contains` [EXTRACTED]
- [[handleSaveSchedule()]] - `contains` [EXTRACTED]
- [[isDateInRange()]] - `contains` [EXTRACTED]
- [[loadAllSchedules()]] - `contains` [EXTRACTED]
- [[loadSchedule()]] - `contains` [EXTRACTED]
- [[toggleAll()]] - `contains` [EXTRACTED]
- [[toggleCategory()]] - `contains` [EXTRACTED]
- [[toggleScheduleCategory()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Reports_Tab
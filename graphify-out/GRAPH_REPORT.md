...
2. `SharePointOpsService` - 96 edges
3. `HRSharePointService` - 25 edges
4. `StrategyService` - 23 edges
5. `PaymentsSharePointService` - 16 edges
6. `useMicrosoftGraph()` - 14 edges
7. `AppsSharePointService` - 14 edges
8. `UserSharePointService` - 14 edges
9. `AssetsSharePointService` - 13 edges
10. `PowerAutomateService` - 13 edges

## Surprising Connections (you probably didn't know these)
- `calculateKpiProgress()` --implements--> `KPI Progress Calculation Logic (Status-Based)`  [INFERRED]
  src\utils\kpiUtils.ts → src/prompts/strategyCalculationLogic.txt
- `calculateKraProgress()` --implements--> `Unit Objective Progress Calculation (Average of KRAs)`  [INFERRED]
  src\utils\kpiUtils.ts → src/prompts/strategyCalculationLogic.txt
- `calculateStrategicProgress()` --implements--> `Strategic Objective Progress Calculation (Average of Unit Objectives)`  [INFERRED]
  src\utils\kpiUtils.ts → src/prompts/strategyCalculationLogic.txt
- `handleSubmit()` --calls--> `handleKpiFormSubmit()`  [INFERRED]
  src\components\kpi\KpiModal.tsx → src\components\unit-tabs\KRAsTab.tsx
- `SCPNG Internal AI Assistant` --semantically_similar_to--> `Elite Legal Systems Architect PNG Capital Markets Specialist`  [INFERRED] [semantically similar]
  src/prompts/scpngDocAnalystPrompt.txt → src/prompts/scpngLawyerAiPrompt.txt

## Hyperedges (group relationships)
- **Both SCPNG AI Prompts Share Identical Follow-Up Questions Tag Format** — scpngDocAnalystPrompt_followup_questions, scpngLawyerAiPrompt_followup_questions, shared_followup_pattern [EXTRACTED 1.00]
- **Strategy Calculation Hierarchy (KPI â†’ Unit Objective â†’ Strategic Objective â†’ Divisional Performance)** — strategyCalculationLogic_kpi_progress, strategyCalculationLogic_unit_objective_progress, strategyCalculationLogic_strategic_objective_progress, strategyCalculationLogic_divisional_performance [EXTRACTED 1.00]
- **OneDrive Upload Pipeline (MSAL Auth â†’ Graph API â†’ Folder Management)** — oneDriveFileUploader_msal_auth, oneDriveFileUploader_graph_api_upload, oneDriveFileUploader_folder_management [EXTRACTED 1.00]
- **KRA/KPI Full Data Flow: UI -> Hooks -> Service -> SharePoint** — src_components_unit_tabs_krastab_tsx, src_components_kpi_kpimodal_tsx, usesharepointops_usesharepointkras, usesharepointops_usesharepointkpis, sharepointopsservice_sharepointopsservice [EXTRACTED 0.95]
- **Progress Calculation Chain: KPI -> KRA -> Objective -> Strategic Goal** — kpiutils_calculatekpiprogress, kpiutils_calculatekraprogress, kpiutils_calculatestrategicprogress, kpiutils_calculategoalprogressfromchildren [EXTRACTED 1.00]
- **KPI Form Component Tree: KpiModal -> KraFormSection + KpiInputBlock** — src_components_kpi_kpimodal_tsx, src_components_kpi_kraformsection_tsx, src_components_kpi_kpiinputblock_tsx, src_components_common_globalassigneeselector_tsx [EXTRACTED 1.00]
...
Cohesion: 0.02
Nodes (8): handleCreateList(), handleDeleteList(), loadLists(), handleITRequestSubmit(), loadRecentITRequests(), useLeaveBalances(), handleFormSubmit(), sanitize()
...
Cohesion: 0.38
Nodes (7): CSV Conversion Utility, Error Handling and Retry Logic, OneDrive Folder Creation and Management, Microsoft Graph API File Upload Endpoint, MSAL Browser Authentication (Silent + Interactive Token Acquisition), OneDriveFileUploader TypeScript Utility Class, Microsoft Graph API (OneDrive + SharePoint)
